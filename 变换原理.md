...
//在看看三角函数的等式,并与其比较：
//先看x'，如果 a = cosb, b = -sinb, c = 0,那么两个等式完全相同。
x' = ax + by + cz
x' = x*cos b - y*sin b

//再看y'，如果 d = sinb, e = cosb, f = 0,那么两个等式完全相同。
y' = dx + ey + fz
y' = x*sin b + y*cos b
...
//这样就可以写出表示平移的矩阵，又称为平移矩阵（translation matrix）。如下所示：
x'   |1 0 0 Tx|   x
y' = |0 1 0 Ty| * y
z'   |0 0 1 Tz|   z
1    |0 0 0 1 |   1
...
//当你通过比较 
x' = xcosb - ysinb与x' = ax + by +cz +d
//可知
a = cosb, b = -sinb, c= 0, d = 0

//以此类推，求得 y'和z'等式中的系数，最终得到4*4的旋转矩阵。如下所示：
x'   |cosb -sinb 0 0|   x
y' = |sinb  cosb 0 0| * y
z'   |0     0    1 0|   z
1    |0     0    0 1|   1
