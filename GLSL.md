...
11. 存储限定字
    ...
    在GLSL ES中，我们经常使用 attriute、varying和uniform限定字来修饰变量，此外，我们有时也会使用const限定字，它表示着色器中的某个变量使恒定的常量。

    | 类型      | 描述 |
    | --------- | ---- |
    | Attribute | attribute变量只能出现在顶点着色器中，只能被声明为全局变量，被用来表示逐顶点的信息。顶点着色器中能够容纳的attribute变量的最大数目与设备有关，你可以通过访问内置的全局常量获取该值。但是，支持WebGL环境都支持至少8个attribute变量。 |
    | uniform   | 可以用在顶点着色器和片元着色器中，且必须是全局变量。uniform变量是只读的，它可以是除了数组或结构体之外的任意类型。如果顶点着色器和片元着色器中声明了同名的uniform变量，那么它就会被两种着色器共享  |
    | varying  | 必须是全局变量，它的任务是从顶点着色器向片元着色器传输数据。我们必须在两种着色器中声明同名、同类型的varying变量。它的类型只能是以下类型：float、vec2、vec3、vec4、mat2、 mat 3、mat4。 |
...
14. GLSL默认值 
    ...
    | gl_Position  | vec4  | 输出属性-变换后的顶点位置，所有的顶点着色器都必须写这个值。 |
    ...
    | 名称         | 类型  | 描述                                                         |
    | ------------ | ----- | ------------------------------------------------------------ |
    | gl_Color     | vec4  | 插值后的主颜色，只读输入                                     |
    | gl_TexCoord  | vec4  | 插值后的纹理坐标，只读输入                                   |
    | gl_FragColor | vec4  | 输出的颜色                                                   |
    | gl_FragCoord | vec4  | 片元的位置，片元相对窗口的坐标位置：x, y, z, 1/w，活动线程正在处理的像素或屏幕碎片的坐标，只读输入 |
    | gl_FragDepth | float | 输出的深度，如果这个值没有被写，则使用固定功能管线的深度值代替 |
