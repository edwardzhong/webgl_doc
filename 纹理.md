...
* param: 纹理参数的值
  ...
  1. 纹理过滤：可赋给 **gl.TEXTURE_MAP_FILTER** 和 **gl.TEXTURE_MIN_FILTER** 参数的值
     ...
     **gl.NEAR_MIPMAP_LINEAR**
   **gl.LINEAR_MIPMAP_NEAREST**
     **gl.LINEAR_MIPMAP_LINEAR**

  2. 纹理包装：可赋给 **gl.TEXTURE_WRAP_S** 和 **gl.TEXTURE_WRAP_T** 的常量：

     如果纹理坐标超出 0.0-1.0 范围，则应用如下包装模式进行平铺重复操作纹理，否则位于区间[0,1]则进行拉伸纹理操作。
  
     **gl.REPEAT**: 平铺式的重复纹理
  
     **gl.MIRRORED_REPEAT**: 镜像对称的重复纹理，用在产生无缝的平铺效果，是为非无缝的纹理而使用(比如右边界不会与左边界很好融合的纹理)
     
     **gl.CLAMP_TO_EDGE**: 使用纹理图像边缘值进行平铺，同样是为非无缝的纹理而准备
