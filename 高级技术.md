...
让物体实现半透明效果需要用到颜色的α分量。该功能被称为a混合(alpha blending) 或 混合 blending，WebGL已经内置该功能，值需要开启，如果只设置了颜色的第四个分量是看不到透明效果的，必须要执行下面两个步骤:
...
实现a混合最简单的方式是屏蔽掉隐藏面消除功能 (去掉 gl.enable(gl.DEPTH_TEST)

但关闭隐藏面消除功能是一个粗暴的解决方案，并不能满足实际需求。实际可通过某些机制，可以同时实现隐藏面消除和半透明效果。我们只需要：
...
void main() {
  ...
  /**
  * 阴影部分
  * 通过光源坐标系下的位置找出阴影纹理的颜色值，再由颜色值rgba分解出深度值z
  */
